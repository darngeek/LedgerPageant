...
class __declspec(uuid("9D0B8B92-4E1C-488e-A1E1-2331AFCE2CB5"))
	LedgerPeageantGUID;
...
void storeControl(HWND hwnd, int control) {
	...
	if (GetWindowRect(controlHandle, &screenRect) != 0) {
		...
		POINT controlSize = {screenRect.right - screenRect.left,
			screenRect.bottom - screenRect.top};
		ScreenToClient(windowHandle, (LPPOINT)&controlPosition);

		controlRects[control] = {controlPosition.x, controlPosition.y,
...
void resizeControls(int clientWidth, int clientHeight) {
	...
	ListView_SetColumnWidth(controlHandles[IDC_LST_IDENTITIES], 1,
...
int AddNewIdentity(HWND listHandle) {
	...
	ident.port = 22;
	int index = Window::GetPtr()->GetApplication()->AddIdentity(ident);
	...
	// Select item in list
	ListView_SetItemState(listHandle, index, LVIS_FOCUSED | LVIS_SELECTED,
		0x000F);
...
bool RemoveIdentityByIndex(HWND listBox, int index) {
	if (index != -1) {
		...
		LPCWSTR description =
			L"Removing this Identity will also remove it from other applications "
			L"like Putty!\n\nAre you sure?";

		int msgboxID = MessageBox(NULL, description, title,
...
bool SaveIdentity(HWND windowHandle, int index, bool changed) {
	if (changed) {
		...
		LPCWSTR description =
			L"Changing an Identity will also change its Public Key\n\nAre you "
			L"sure?";
...
BOOL CALLBACK ManageIdentProc(HWND hwnd, UINT Message, WPARAM wParam,
	...
	case WM_INITDIALOG:
	{
		...
		lvc.mask = LVCF_FMT | LVCF_WIDTH | LVCF_TEXT |
			LVCF_SUBITEM;  // format, width, text, subitem are valid:
		lvc.fmt = LVCFMT_LEFT;
		...
		LoadString(g_hInst, IDS_LST_COL_PUBKEY, columnPubKey,
			ARRAYSIZE(columnPubKey));
		...
		ListView_SetItemState(m_hListBox, app->GetNumIdentities() - 1,
	...
	case WM_NOTIFY:
	{
		...
		case LVN_GETDISPINFO:
		{
			...
			if (plvdi->item.iSubItem == 0) {
				HRESULT hr = StringCchCopy(plvdi->item.pszText,
					ident.name.size() * sizeof(WCHAR),
					(LPWSTR)ident.name.c_str());
			}
		...
		if (VAL_notify->hdr.code == LVN_ITEMCHANGED &&
			VAL_notify->hdr.idFrom == IDC_LST_IDENT &&
			(VAL_notify->uNewState & LVIS_SELECTED)) {
			// get identity for selected item
			int iPos = ListView_GetNextItem(m_hListBox, -1, LVNI_SELECTED);
			Identity ident = Window::GetPtr()->GetApplication()->GetIdentityByIndex(iPos);
	...
	case WM_CONTEXTMENU:
	{
		...
		if ((HWND)wParam == m_hListBox) {
			HMENU m_hListMenu =
				LoadMenu(g_hInst, MAKEINTRESOURCE(IDC_LSTIDENT_MENU));
			if (m_hListMenu) {
	...
	case WM_COMMAND:
	{
		...
		case IDC_BTN_GETKEY:
			...
			break;
		case IDC_BTN_REMOVE:
			RemoveIdentityByIndex(
				m_hListBox,
				ListView_GetNextItem(m_hListBox, -1, LVNI_ALL | LVNI_SELECTED));
			break;
		case IDC_BTN_SAVE:
			SaveIdentity(
				hwnd,
	...
	case WM_GETMINMAXINFO:
		((LPMINMAXINFO)lParam)->ptMinTrackSize.x =
			windowStartScreenSize.right - windowStartScreenSize.left;
		((LPMINMAXINFO)lParam)->ptMinTrackSize.y =
			windowStartScreenSize.bottom - windowStartScreenSize.top;
		break;
	case WM_CLOSE:
		// DestroyWindow(hwnd);
		EndDialog(hwnd, 0);
...
LRESULT CALLBACK NotificationIconCallback(HWND hwnd, unsigned int message,
	...
	case WM_COMMAND:
	{
		...
		case IDM_MNGIDENT:
		{
			...
			if (windowHandle == NULL) {
				DialogBox(g_hInst, MAKEINTRESOURCE(IDD_MNGIDENT), hwnd,
					(DLGPROC)ManageIdentProc);
			}
			else {
...
void RegisterWindowClass(HINSTANCE instance, LPCWSTR pszClassName,
	LPCWSTR pszMenuName, WNDPROC lpfnWndProc) {
...
HWND Window::Make(HINSTANCE instance) {
	g_hInst = instance;
	RegisterWindowClass(instance, szWindowClass,
	...
	HWND retHandle =
		CreateWindow(szWindowClass, szTitle, WS_OVERLAPPEDWINDOW, CW_USEDEFAULT,
			0, 250, 200, NULL, NULL, g_hInst, NULL);
